...
int misc_init_r(void)
{
	...
	char cmd[64] = { '\0' };
#elif defined(NEST_BUILD_CONFIG_COMPLIANCE)
...
#error "A specific Nest Diamond board type has not been configured!"
	...
	if ((p == NULL) || (simple_strtol(p, NULL, 10) != 1)) {
		cmd = "mw.l 0x80000000 0 0xfbbfa2 && " // zero out memory up to around the stack pointer
					"nand erase 0xc00000  0x2e00000 && " // root0
					"nand erase 0x9000000 0x1400000 && " // log
					"env set compliance 1 && " // Set the compliance flag to know this was done
					"env set preboot && "
					"saveenv";
	}
#endif
